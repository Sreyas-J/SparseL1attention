"""
EcoTransformer — L1 Longformer: Multi-Precision Evaluation Script (IMDb)
=========================================================================
Loads a saved L1-Longformer model (from longformer_l1_imdb.py) and
evaluates it on the IMDb test set across four precisions:
...
Usage:
    python longformer_imdb_eval.py
    python longformer_imdb_eval.py --model_path ./best_longformer_l1_imdb
    python longformer_imdb_eval.py --batch_size 16 --skip_int8
...
# ══════════════════════════════════════════════════════════════════════════════
# Model definition — identical to longformer_l1_imdb.py
...
class IMDbDataset(Dataset):
    ...
    seq_len is padded/truncated to MAX_LENGTH=512 which satisfies
    the 2×attention_window=512 constraint.
    """
...
def print_table(results: list):
    W = 84
    print("\n" + "=" * W)
    print(f"{'PRECISION COMPARISON — L1-Longformer on IMDb Test Set':^{W}}")
    print("=" * W)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Multi-precision eval for L1-Longformer on IMDb")
    p.add_argument("--model_path", default="./best_longformer_l1_imdb",
                   help="Path to saved model dir (default: ./best_longformer_l1_imdb)")
