"""
EcoTransformer — L1 BigBird: Multi-Precision Evaluation Script (IMDb)
======================================================================
Loads a saved L1-BigBird model (from bigbird_imdb.py) and evaluates it
on the IMDb test set across four precisions:
...
IMDb-specific details:
  - Dataset field is "text" (not "sentence" as in SST-2)
  - Test split used as evaluation (25,000 samples)
  - Default MAX_LENGTH=512 → BigBird uses original_full path
    (block_sparse requires seq_len ≥ 704; set --max_length 1024 to trigger it)
  - No global_attention_mask needed (BigBird manages global tokens internally)

Usage:
    python bigbird_imdb_eval.py
    python bigbird_imdb_eval.py --model_path ./best_bigbird_l1_imdb
    python bigbird_imdb_eval.py --batch_size 8 --max_length 512
    python bigbird_imdb_eval.py --skip_int8
    python bigbird_imdb_eval.py --skip_fp16

Requirements:
    pip install transformers datasets torch bitsandbytes sentencepiece
"""
...
MAX_LENGTH = 512   # <704 → original_full; ≥1024 → block_sparse
L1_LAMBDA  = -0.1


# ══════════════════════════════════════════════════════════════════════════════
# Model definition — identical to bigbird_imdb.py (training script).
...
class L1BigBirdBlockSparseAttention(BigBirdBlockSparseAttention):
    ...
    def bigbird_block_sparse_attention(
        ...
        attention_probs = (
            torch.zeros(bsz, n_heads, from_seq_len, to_seq_len,
...
# ══════════════════════════════════════════════════════════════════════════════
# Dataset — IMDb uses "text" field (not "sentence" as in SST-2)
# ══════════════════════════════════════════════════════════════════════════════

class IMDbDataset(Dataset):
    """
    IMDb wrapper for BigBird evaluation.

    Key difference from SST2Dataset: field name is "text" not "sentence".
    Test split (25k samples) is used as the evaluation set.
    """
    ...
    def __getitem__(self, idx):
        enc = self.tokenizer(
            self.data[idx]["text"],          # IMDb uses "text", not "sentence"
            truncation=True,
            padding="max_length",
...
def evaluate_model(model, loader, device,
    ...
    """
    Run full evaluation loop for one precision setting.
    ...
    BigBird note: no global_attention_mask needed — global tokens are handled
    internally via BigBird's first/last block design.
    """
    model.eval()
...
def print_table(results: list):
    W = 84
    print("\n" + "=" * W)
    print(f"{'PRECISION COMPARISON — L1-BigBird on IMDb Test':^{W}}")
    print("=" * W)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Multi-precision eval for L1-BigBird on IMDb")
    p.add_argument("--model_path", default="./best_bigbird_l1_imdb",
                   help="Path to saved model dir (default: ./best_bigbird_l1_imdb)")
    p.add_argument("--batch_size", type=int, default=4,
                   help="Eval batch size (default: 4; reduce for max_length=1024)")
    p.add_argument("--max_length", type=int, default=512,
                   help="Token length — must match training (default: 512). "
                        "BigBird uses original_full for seq_len < 704, "
                        "block_sparse for seq_len >= 704.")
...
def main():
    ...
    block_sparse_threshold = (5 + 2 * 3) * 64   # = 704
    ...
    print(f"Attention mode: {attn_mode} "
          f"(seq_len={args.max_length}, threshold={block_sparse_threshold})")

    # ── Tokenizer ─────────────────────────────────────────────────────────────
    tokenizer = BigBirdTokenizer.from_pretrained(args.model_path)

    # ── Dataset — IMDb test split (25k samples) ───────────────────────────────
    ...
    loader   = DataLoader(val_data, batch_size=args.batch_size,
                          ...
                          pin_memory=True)
    print(f"Test samples : {len(val_data):,}  |  Batches: {len(loader):,}")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 3. FP16 — float16 (may overflow for long IMDb sequences)
    # ══════════════════════════════════════════════════════════════════════════
    print("\n── [3/4] FP16 ───────────────────────────────────────────────────────")
    if args.skip_fp16:
        ...
        if r["nan_batches"] or r["inf_batches"]:
            ...
            print("     especially for long IMDb sequences. Use BF16 instead.")
        del model; reset_gpu_stats()
