...
#
# Script generated by Vivado on Sun Jan 25 19:32:41 IST 2026
...
#
# Usage        : ACC.sh
#                ACC.sh [-lib_map_path] [-step] [-keep_index] [-noclean_files]*
#                ACC.sh [-reset_run]
#                ACC.sh [-reset_log]
#                ACC.sh [-help]
#
...
#                script (type 'ACC.sh -help' for more information).
#
...
# script info
echo -e "ACC.sh - Script generated by export_simulation (Vivado v2023.2 (64-bit)-id)\n"
...
run()
{
  ...
  if [[ ($b_step == 1) ]]; then
    case $step in
      ...
      ;;
      * )
        echo -e "ERROR: Invalid or missing step '$step' (type \"./ACC.sh -help\" for more information)\n"
        exit 1
      esac
  else
    init_lib
...
elaborate()
{
  vcs $vcs_elab_opts xil_defaultlib.ACC xil_defaultlib.glbl -o ACC_simv
}

# RUN_STEP: <simulate>
simulate()
{
  ./ACC_simv $vcs_sim_opts -do simulate.do
}
...
create_lib_mappings()
{
  ...
  if [[ ($lib_map_path == "") ]]; then
    lib_map_path="/home/sk/Desktop/SparseL1Attention/AttentionCore/AttentionCore.cache/compile_simlib/vcs"
  fi
...
reset_run()
{
  files_to_remove=(ucli.key ACC_simv vlogan.log vhdlan.log compile.log elaborate.log simulate.log .tmp_log .vlogansetup.env .vlogansetup.args .vcs_lib_lock scirocco_command.log lib_sc.so 64 AN.DB csrc ACC_simv.daidir vcs_lib c.obj)
...
check_arg_value()
{
  ...
  if [[ ($1 == "-lib_map_path") && ($2 == "") ]];then
    echo -e "ERROR: Simulation library directory path not specified (type \"./ACC.sh -help\" for more information)\n"
    exit 1
  fi
}
...
usage()
{
  msg="Usage: ACC.sh [-help]\n\
Usage: ACC.sh [-step]\n\
Usage: ACC.sh [-lib_map_path]\n\
Usage: ACC.sh [-reset_run]\n\
Usage: ACC.sh [-reset_log]\n\
Usage: ACC.sh [-keep_index]\n\
Usage: ACC.sh [-noclean_files]\n\n\
